...
///
/// NOTE: Enum and docs generated by Gemini. Feel free to submit a pull request if any mistakes are
/// seen.
#[subenum(AdvtAction)]
...
pub enum StandardAction {
    ...
    /// Evaluates a set of conditional statements in the LaunchCondition table that
    /// must all be true for the installation to proceed.
    ...
    /// Queries the Patch table to determine which patches are applied to specific
    /// files and then performs the byte-wise patching of the files.
    PatchFiles = 4090,
    ...
    /// Registers product information with the installer and stores the installer
    ...
    StopServices = 1900,
    /// Manages the unadvertisement of components from the PublishComponent table
    /// and removes information about published components.
