...
pub fn generate_dao_tokens(
    ...
    let dao_struct_tokens =
        generate_dao_struct_definition(&dao_struct_ident, fields);
    let dao_impl_tokens = generate_new_for_dao(target_name, fields);
    let primary_identifier_impl_tokens =
        generate_primary_identifier_impl_definition(
            primary_identifier,
            &dao_struct_ident,
        );
...
fn generate_dao_struct_definition(
    ...
    // Pretty sure we could just append `fields` to the token stream for this
...
fn generate_msi_dao_impl_definition(
    ...
    let conflicts_definition_tokens =
        generate_msi_dao_conflicts_definition(fields);
...
fn generate_msi_dao_conflicts_definition(
    ...
    // Get the fields that are marked as primary_key as these are what is used
...
fn generate_msi_dao_to_row_definition(
    fields: &Vec<FieldInformation>,
) -> TokenStream {
