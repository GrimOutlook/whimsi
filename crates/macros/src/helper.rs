...
// NOTE: I can already see a bug here but I don't care at the moment. If the
// word starts with an `_` the `_` will be correctly preserved but since the
// first character in the string is `_` no capitalization will occur, when
// likely you would want the first letter after it to be capitalized. Maybe I'll
// fix it later :p
