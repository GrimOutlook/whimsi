...
impl FromStr for Version {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // Yes I know it's not actually an octet since there aren't 8 but people still call each
        // part of an IP address an octet :p
        let octets = s.split(".");
        ...
        /// Verify that each octet can be represented as a u16 since version octets cannot be a
...
pub enum VersionError {
    ...
    #[error("Octet [{0}] at index [{1}] is is not a number. Only numbers allowed in MSI version.")]
