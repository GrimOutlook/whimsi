...
impl DirectoryTable {
    ...
    fn add(&mut self, directory: Directory) -> anyhow::Result<()> {
        for child in directory
            .contained()
            .iter()
            .filter_map(|node| node.try_as_directory_ref())
        {
            let dir: Directory = (*child.borrow()).clone().into();
            self.0.push(DirectoryDao::new(&dir, &directory)?);
            self.add(dir)?;
        }
        Ok(())
    }
